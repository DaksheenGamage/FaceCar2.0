package com.example.facecar20.FaceDetection;
...
public class MTCNN {
    ...
    //read  pixel values of the Bitmap, Pretreatment(-127.5 /128)，Convert to a one-dimensional array and return
    ...
    /*
       Face detection,minSize is the smallest face pixel value
     */
    private Bitmap bitmapResize(Bitmap bm, float scale) {
        int width = bm.getWidth();
        int height = bm.getHeight();
        // CREATE A MATRIX FOR THE MANIPULATION。matrix specify image affine transformation parameters
    ...
    private  int PNetForward(Bitmap bitmap, float [][]PNetOutProb, float[][][]PNetOutBias){
        ...
        Utils.flip_diag(PNetIn,h,w,3); //Flip diagonally
    ...
    private void nms(Vector<Box> boxes, float threshold, String method){
        ...
        for(int i=0;i<boxes.size();i++) {
            Box box = boxes.get(i);
            if (!box.deleted) {
                //score<0 means the current rectangle is deleted
    ...
    //Intercept box rectangular frame specified in (cross-border to be processed)，and resize to size*size.
